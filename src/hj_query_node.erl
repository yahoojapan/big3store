...
%% 
%% @doc Hash-join query node is implemented as independent gen_process. Hash-join query node is 
%% a state-machine realizing hash-join algorithm as protocol among join query node and the outer
%% and inner query nodes. This implementation of hash-join algorithm expects that 
%% there is enough main memory that all graphs from outer query nodes are stored in index.
%%
%% <table bgcolor="lemonchiffon">
%% <tr><th>Section Index</th></tr>
%% <tr><td>{@section hash join algorithm}</td></tr>
...
%% == hash join algorithm ==
%% 
%% (LINK: {@section hash join algorithm})
%% 
%% Join query node is independent gen_server process that can have multiple 
%% outer query nodes as well as multiple inner query nodes--each of them is implemented as 
%% separate gen_server process.
%% 
%% A variant of hash-join algorithm is implemented. It retrieves all results from outer query 
%% node and creates index in main memory based on join attributes. Each graph from inner 
%% query node is joined with outer graphs from index that have the same values of join 
%% attributes.
%%
...
%% to wait_next_outer, the first phase of algorithm. In this phase hash-join reads results
%% from outer query nodes and creates in-memory index on join attributes. After reading 
%% all results from outer query node protocol enters second phase and state is changed
%% wait_next_inner.
%%  
%% In the second phase of protocol, results from inner query nodes are joined with 
%% outer triples by using in-memory index. After end_of_stream is received from all inner
%% query nodes, state of protocol moves to eos.
%%
%% == property list ==
%% 
...
%% are sent to each of outer processes when eval message of hj_query_node
%% is processed.</td> </tr>
%% 
...
%% init/1
%% 
%% @doc Initialize a hj_query_node process.
%% 
...
%% 
child_spec(Id) ->
    GSOpt = [{local, Id}, hj_query_node, [], []],
    StartFunc = {gen_server, start_link, GSOpt},
    Restart = permanent,
    Shutdwon = 1000,
    Type = worker,
    Modules = [hj_query_node],
...
spawn_process(Id, Node ) -> 
    ChildSpec = hj_query_node:child_spec(Id),
...
%% hc_start/10
%%
%% @doc Initialize hj_query_node process. 
%% 
...
hcst_q01() ->
    ...
    JQN1          = hj_query_node:spawn_process(Id, node()),
...
hc_data_outer(State) when State == wait_next_outer -> 
    ...
    %% outer loop actions for Graph read from outer queue 
    case Graph of 
    end_of_stream -> 
    ...
    {wait_next_inner, false} ->
          ...
          %% outer graphs have been processed and eval has been initiated for all inner qns.
    ...
    case {get(state), QEFO} of 
    {wait_next_outer, false} ->
         %% outer queue not empty yet, continue with next outer graph
         hc_data_outer(get(state)),
...
hcdo_process_eos(From) ->
    ...
    case NumAlive of 
    0 -> %% activate inner streams and move to send parent eos and set state eos
         hcdo_activate_inner(),
...
hcdo_process_graph(Graph) ->
    ...
    OI = get(index_outer), 
    case maps:is_key(JV, OI) of 
    true  -> L = maps:get(JV, OI), 
             put(index_outer, maps:put(JV, [Graph|L], OI));
    false -> put(index_outer, maps:put(JV, [Graph], OI))
    end.
...
hc_data_inner(State) when State =:= wait_next_inner ->
    ...
    %% inner loop actions for inner graph read from queue
    case Graph of
    end_of_stream ->
...
hcdi_send_parent_eos() ->
    ...
    true  -> %% there is empty message from parent
             ...
             gen_server:cast(get(parent), Msg),
             info_msg(send_cast, [get(self), {message,Msg}, {to,get(parent)}, {invoker,hcdi_send_parent_eos}, get(state)], message_sent, 30);
...
hcdi_match_outer_graphs() ->
    ...
    %% store inner graph in pd
    Graph = get(inner_graph),
    ...
    %%info_msg(hcdo_match_outer_graphs, [get(self), {inner_graph,Graph}, {join_vars,get(join_vars)}, {vars_pos,get(vars_pos)}, get(state)], debug_join_var_values, 20),
    JV = lists:map(F1, get(join_vars)),
    %%info_msg(hcdo_match_outer_graphs, [get(self), {inner_graph,Graph}, {join_vars,get(join_vars)}, {join_values,JV}, get(state)], debug_join_var_values, 20),

    %% attempt to join inner Graph to outer_index
    OI = get(index_outer), 
    case maps:is_key(JV, OI) of 
    true  -> hcdi_process_graph(maps:get(JV, OI));
    false -> ok
    end.
    
hcdi_process_graph([OG|GList]) ->
    ...
    hcdi_process_graph(GList);

hcdi_process_graph([]) -> ok.
...
hcet_q02() ->
    ...
    JQN3          = hj_query_node:spawn_process(Id3, node()),
...
hcet_q03() ->
    ...
    JQN5          = hj_query_node:spawn_process(Id5, node()),
...
hcet_q05() ->
    ...
    R1            = {DFO, JQN5, [R3Map,R3Map,R3Map,R3Map,R2Map]},
    R2            = {DFO, JQN5, [R2Map,R2Map,R2Map,R1Map,R1Map]},
    R3            = {DFO, JQN5, [R1Map,R1Map,R3Map,R3Map,R3Map]},
    R4            = {DFO, JQN5, [R3Map,R2Map,R2Map,R2Map,R2Map]},
    R5            = {DFO, JQN5, [R1Map,R1Map,R1Map,R1Map,EOS]},
...
hcet_q06() ->
    ...
    R2Map         = maps:put("4", T5, maps:put("2", T4, maps:put("1", T1, maps:new()))),
    R3Map         = maps:put("4", T7, maps:put("2", T6, maps:put("1", T1, maps:new()))),
    R1            = {DFO, JQN5, [R3Map,R3Map,R3Map,R3Map,R3Map]},
    R2            = {DFO, JQN5, [R3Map,R3Map,R3Map,R3Map,R2Map]},
    R3            = {DFO, JQN5, [R2Map,R2Map,R2Map,R2Map,R2Map]},
    R4            = {DFO, JQN5, [R2Map,R2Map,R2Map,R1Map,R1Map]},
    R5            = {DFO, JQN5, [R1Map,R1Map,R1Map,R1Map,R1Map]},
    R6            = {DFO, JQN5, [R1Map,R1Map,R3Map,R3Map,R3Map]},
    R7            = {DFO, JQN5, [R3Map,R3Map,R3Map,R3Map,R3Map]},
    R8            = {DFO, JQN5, [R3Map,R2Map,R2Map,R2Map,R2Map]},
    R9            = {DFO, JQN5, [R2Map,R2Map,R2Map,R2Map,R2Map]},
    R10           = {DFO, JQN5, [R1Map,R1Map,R1Map,R1Map,R1Map]},
    R11           = {DFO, JQN5, [R1Map,R1Map,R1Map,R1Map,R3Map]},
    R12           = {DFO, JQN5, [R3Map,R3Map,R3Map,R3Map,R3Map]},
    R13           = {DFO, JQN5, [R3Map,R3Map,R3Map,R2Map,R2Map]},
    R14           = {DFO, JQN5, [R2Map,R2Map,R2Map,R2Map,R2Map]},
    R15           = {DFO, JQN5, [R2Map,R2Map,R1Map,R1Map,R1Map]},
    R16           = {DFO, JQN5, [R1Map,R1Map,R1Map,R1Map,R1Map]},
...
%   info_msg(hcet_q06, [get(self), R1, RE], before_tests, 50),
    ...
    {inorder,
     [
      ...
      ?_assertMatch({'$gen_cast', R10},      hcet_send_empty(JQN5, R10)),
      ?_assertMatch({'$gen_cast', R11},      hcet_send_empty(JQN5, R11)),
      ?_assertMatch({'$gen_cast', R12},      hcet_send_empty(JQN5, R12)),
      ?_assertMatch({'$gen_cast', R13},      hcet_send_empty(JQN5, R13)),
      ?_assertMatch({'$gen_cast', R14},      hcet_send_empty(JQN5, R14)),
      ?_assertMatch({'$gen_cast', R15},      hcet_send_empty(JQN5, R15)),
      ?_assertMatch({'$gen_cast', R16},      hcet_send_empty(JQN5, R16)),
